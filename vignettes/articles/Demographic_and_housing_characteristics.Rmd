...
For example, the single variable "H12B_010N" reports the number of households
that are headed by a Black, non-Hispanic person who is renting, not an owner.
Those nine characters therefore encode information about race, ethnicity, and
home ownership.
The value of the variable is another piece of information, the number of
households.
...
Each glossary will need enough columns to represent all of the details that are
captured by its rows.
...
Most of the groups will need two or fewer columns to capture their details,
but many will need more.
Three different tables will need five columns!
...
That is what the `Level of Aggregation` field is intended to capture.
